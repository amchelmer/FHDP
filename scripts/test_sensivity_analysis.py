...
PITCH_ADD_METHOD = "clone-gauss"
PITCH_DOT_ADD_METHOD = "clone-gauss"
...
def train(args):
    ...
    actor_critic.change_feature(
        FeatureChange(feature_theta, PITCH_ADD_METHOD, spread=PITCH_SPREAD)
    )

    actor_critic.set_memory_sizes(STAGE_TWO_AC_MEMORY, STAGE_TWO_AC_MEMORY, STAGE_TWO_PM_MEMORY)
    actor_critic.change_feature(
        FeatureChange(feature_thetadot, PITCH_DOT_ADD_METHOD, spread=PITCH_DOT_SPREAD)
    )
...
def lower_alpha_a():
    ...
    print("Starting training of quad-rotor with lower alpha_a.")
    ...
    print("Finished lower alpha_a with {:.2f} (id={})".format(
        SimulationResult(
    ...
    trained_cs.notes = "Sensitivity analysis: lower alpha_a"
    # trained_cs.dump()
...
def higher_alpha_a():
    ...
    print("Starting training of quad-rotor with higher alpha_a.")
    trained_cs = ControllerSet(
        parallelize(
            parsed_args.j,
            train,
    ...
    print("Finished higher alpha_a with {:.2f} (id={})".format(
        SimulationResult(
            trained_cs.lookback_result(LOOK_BACK_WINDOW),
            metric=parsed_args.metric
        ).get_cum_state_error()[1:2].sum(),
    ...
    trained_cs.notes = "Sensitivity analysis: even higher alpha_a of 9x"
    # trained_cs.dump()
    RewardSet(trained_cs).dump()


def lower_discount_rate():
    quad_rotor_plant = QuadRotor2DPlant(
        1. / FREQUENCY,
        blade_flapping=BLADE_FLAPPING,
        init_mean=DEFAULT_INIT_STATE_MEAN,
    ...
    print("Starting training of quad-rotor with lower discount rate.")
    trained_cs = ControllerSet(
        parallelize(
            parsed_args.j,
            train,
    ...
    print("Finished lower discount rate with {:.2f} (id={})".format(
        SimulationResult(
            trained_cs.lookback_result(LOOK_BACK_WINDOW),
            metric=parsed_args.metric
        ).get_cum_state_error()[1:2].sum(),
    ...
    trained_cs.notes = "Sensitivity analysis: lower discount rate"
    # trained_cs.dump()
    RewardSet(trained_cs).dump()
